...
class CreatePoints(object):
	...
	axbox = plt.axes([0.15, 0.02, 0.1, 0.075])
	self.text_box = TextBox(axbox, 'Clase Actual', initial='')
	...
	#esta accion debe corresponderse a un boton, cuidado con esto
        if event.button == 3:
	...
	if x0 > 5 and y0 > 5: #esto es necesario por un problema con el textbox que marca puntos al hacer click para escribir
